    ...
    let
      ...
      perSystem =
        ...
        let
          ...
          seRuntimeLibs = lib.makeLibraryPath [
            pkgs.cairo
            pkgs.gdk-pixbuf
            pkgs.glib
            pkgs.pango
              ...
              ''
                ...
                for bin in ${seVenv}/bin/se ${seVenv}/bin/se-*; do
                  ...
                  makeWrapper "$bin" "$out/bin/$name" \
                    --prefix PATH : ${lib.makeBinPath seBrowserDeps} \
          ...
          se-ext = pkgs.stdenv.mkDerivation {
            ...
            installPhase = ''
              ...
              makeWrapper $out/bin/ext.sh $out/bin/se-ext \
                --prefix PATH : ${lib.makeBinPath seExtRuntimeDeps} \
        ...
        in
        {
          ...
          devShells.default = pkgs.mkShell {
            ...
            shellHook = ''
              ...
              echo ""
              echo "Starting a new book? Scaffold .envrc, .gitignore, Justfile, and"
              echo "CLAUDE.md in one step with: se-ext init"
              echo ""
      ...
      perSystemOutputs = forAllSystems perSystem;
    in
    {
      packages = forAllSystems (system: perSystemOutputs.${system}.packages);
      apps = forAllSystems (system: perSystemOutputs.${system}.apps);
